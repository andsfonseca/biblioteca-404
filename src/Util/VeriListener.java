	...
	public void afterPhase(PhaseEvent event) {
		...
		FacesContext con = FacesContext.getCurrentInstance();
		 if("/index.xhtml".equals(context.getViewRoot().getViewId()) || 
			...
			PessoasBean bean = (PessoasBean) context.getApplication().evaluateExpressionGet(context, "#{bPessoa}", PessoasBean.class) ;
		 
		 
		 if(!bean.logado()){
			 NavigationHandler nv = context.getApplication().getNavigationHandler();
			 nv.handleNavigation(context, null, "index?faces-redirect=true");
			 con.addMessage(null, new FacesMessage("Erro!" , "É nececessário estar logado para acessar essa página") );
